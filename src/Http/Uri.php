...
class Uri implements UriInterface
{
    ...
    public function __construct($uri = null)
    {
        if (!is_null($uri)) {
            $parts = parse_url($uri);
            if (false == $parts) {
                throw InvalidArgumentException::InvalidUrl($uri);
            }
    ...
    public function withUserInfo($user, $password = null)
    {
        if (!is_string($user) && !is_null($user)) {
            throw InvalidArgumentException::InvalidType('user', 'string', $user);
        }
    ...
    {
        if (!is_string($host)) {
            throw InvalidArgumentException::InvalidType('host', 'string', $host);
        }
    ...
    public function withPort($port)
    {
        if (!is_int($port) && !is_null($port)) {
            throw InvalidArgumentException::InvalidType('port', 'int', $port);
        }
        if ($port < 0 || $port > 61000) {
            throw InvalidArgumentException::OutOfRange('port', '0-61000', $port);
        }
        $new = clone $this;
        $new->port = $port;
    ...
    {
        if (!is_string($query)) {
            throw InvalidArgumentException::InvalidType('query', 'string', $query);
        }
    ...
    public function withFragment($fragment)
    {
        if (!is_string($fragment)) {
            throw InvalidArgumentException::InvalidType('fragment', 'string', $fragment);
        }
